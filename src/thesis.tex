...
following we try to be as informal as is permitted by the necessity to address
and represent all the essential size problems in our subjects. In the following,
we describe the new universe setup, and adapt the previously described
signatures and semantics to it.
...
We instead chose to merge them into the current $\blank^T$, since we don't use
...
%% Also, the
%% Böhm-Beraducci encodings in Section \ref{sec:bb-encoding} will support recursion
%% but not elimination, so it makes sense to try to ``warm up'' to that case as
%% well.
...
lowest possible level, to any other universe. So far we haven't introduced a
...
%% We don't have to assume ToS syntax in lambda-encodings, but we still need to
...
%% universe as $\Con$, $\Sub$ and $\Ty$. In our case, that's $\U_{(j \lub 1) + 1}$,
...
for all bootstrap signatures, since we cannot do induction on them, and we don't
...
equalities are available, and unlike in basic intensional type theory, they can
be also used in interesting ways, thanks to univalence.

In short, we might think of HIITs as QIITs in homotopy type theory, or
alternatively, QIITs are specific HIITs that are set-truncated. Thus HIITs are
more general than QIITs, modulo some minor details.

We shall see that 2LTT is firmly necessary in the specification of HIITs,
and the ToS must live in the outer layer. The reason is that there is no known
way to sensibly internalize the metatheory of type theories purely inside
HoTT. This is the problem of ``HoTT eating itself'' \cite{hott-eat-itself}. It
is also closely related to the problem of representing semisimplicial types
purely inside HoTT. If we can construct semisimplicial types in an embedded type
theory, and interpret that into non-truncated HoTT types, that would indeed
solve the problem. But so far it has not been solved, or proven impossible. A
main point of 2LTT is precisely to allow construction of semisimplicial types
\cite{twolevel}.

\subsubsection{Overview}

This chapter is based on the author's joint work with Kaposi \cite{hiits}. In
the following, we specify the ToS and give a semantics of signatures which is
more limited than what we had before. Concretely, we restrict ourselves to the
AMDS fragment of semantics. This is sufficient to compute what we mean by
induction and initiality (which has been called ``homotopy initiality'' in the
context of HoTT \cite{homotopy-initial}).

Why don't we go further? The main reason is that the natural semantics is
...
\cite{hiits} we gave an example that a naive attempt to extend the AMDS
...
We do note that the higher semantics has been done by Capriotti and Sattler. See
\cite{capriotti2020higher} for an abstract; the bulk of the work remains
unpublished as of now. In short, Capriotti and Sattler define ToS in 2LTT, and
also use 2LTT to give a model where signatures are higher categories specified
as complete Segal types. They show that categories of algebras have finite
limits and that initiality is equivalent to induction. Additionally, the setup
yields a structure identity principle for each signature. However, reductions to
simpler type formers are not discussed, nor possible term algebra
...
and \emph{intensional} identity $\blank\!=\!\blank$. Thus, unlike before, the
inner theory is intensional and compatible with univalence. We use the ``based''
induction principle for $\blank\!=\!\blank$ \cite[Section~1.12.1]{hottbook}:
\begin{alignat*}{3}
  & \ms{J} :\,&&\{A : \Ty_0\}\{x : A\}(B : (y : A) \to x = y \to \Ty_0) \to B\,x\,\refl\\
  &           &&\to (y : A)(p : x = y) \to B\,y\,p\\
  & \rlap{$\ms{J\beta} : \ms{J}\,B\,\mi{br}\,x\,\refl \equiv \mi{br}$}
\end{alignat*}

\section{Theory of Signatures}

The theory of HII signatures is fairly similar to the theory of infinitary QII
signatures. The primary difference is that $\Id$ is intensional in the ToS too: it
has $\refl$ and $\ms{J}$, and does not have equality reflection.

However, the choice of $\beta$-rules is now a bit more delicate. We have two
options in the specification of the ToS.
\begin{enumerate}
\item
  One choice is to have definitional $\beta$-rules for $\Id$ and $\Pi$,
  expressed using $\blank\!\equiv\!\blank$. However, this forces us to have
  strict equalities in the $\blank^M$ and $\blank^S$ interpretations of $\El$,
  in other words, to have algebra morphisms and sections which preserve
  structure definitionally. As we will see, if we instead use inner
  (``fibrant'') equality for structure-preservation, the $\blank^M$ and
  $\blank^S$ interpretations fail to respect the mentioned $\beta$-rules in the
  ToS.

  Now, such strict morphisms and sections of algebras are sensible and useful
  when we want to specify HIITs, because $\beta$-rules of recursors and
  eliminators are generally expected to be definitional in HoTT. But they are
  not appropriate when we want to internally talk about algebra morphisms in
  HoTT, as strict equations are only available in 2LTT. For example, it only
  makes sense to specify a structure identity principle with ``weak'' algebra
  morphisms, which preserve structure up to inner equality.

\item
  The other option is to drop strict $\beta$ for $\Pi$ and $\Id$. In this case,
  we can still specify $\beta$ for $\Id$ using $\Id$ itself, but we cannot
  specify $\beta$ and $\eta$ for $\Pi$ at all, because $\Id$ can only express
  equality of small terms in some $\Tm\,\Gamma\,(\El\,a)$. We \emph{could} add a
  ``big'' version of $\Id$ for this purpose, but this feels like an unnecessary
  complication. Instead, for the sake of simplicity, we just drop $\lam$ for
  $\Pi$ together with the $\beta\eta$ rules. This has a negligible impact on the
  practical expressiveness of signatures, although the ToS expressiveness for
  specifying model constructions does take a hit.

  In summary, we only have $\app$ for $\Pi$, and a weak $\beta$-rule for
  $\ms{J}$.  This works fine in the semantics, as $\blank^M$ and $\blank^S$ do
  preserve weak $\ms{J}\beta$ up to inner equality.
\end{enumerate}
We shall refer to option 1 as \textbf{strict ToS}, and option 2 as \textbf{weak
  ToS}. In the following we present the two options simultaneously.

\begin{mydefinition}
A \textbf{model of ToS} (weak or strict) consists of a base cwf, with underlying
sets returning in $\Set$, supporting the following type formers.
  \begin{itemize}
    \item A \textbf{cwf} with underlying sets $\Con$, $\Sub$, $\Ty$ and $\Tm$, all returning in
      the outer $\Set$ universe of 2LTT.
    \item A \textbf{Tarski-style universe} $\U$ with decoding $\El$. $\U$ is closed under the following type formers:
