...
%% $\U$ is intensional, with the following rules:
%% \begin{alignat*}{3}
%%   & \J &&: \{t : \Tm\,\Gamma\,(\El\,a)\}
...
%% expressions internal to the theory of signatures. This is not possible in our
%% signatures. The simplest example is rose trees:
%% \begin{alignat*}{3}
%%   &\ms{Tree} &&: \Set\\
%%   &\ms{node} &&: \ms{List}\,\ms{Tree} \to \ms{Tree}
%% \end{alignat*}
%% We assume $\ms{List} : \Set \to \Set$. The above listing is not a signature,
%% since $\ms{List}\,\ms{Tree}$ is not representable in a signature; the
...
tabular manner. For each interpretation, we omit equations and substitution for
type formers. We discuss some of the more interesting cases.
...
\end{alignat*}
This is the same as before in Definition \ref{TODO}, except we do not have an
flcwf of algebras. Hence, we do not have equivalence of initiality and
induction, nor other properties that are evident in an flcwf, like Lemma \ref{TODO}.
\end{mydefinition}
