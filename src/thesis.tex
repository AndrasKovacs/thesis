...
\begin{itemize}
  ...
  \item Bootstrapping the ToS syntax as ``raw'' syntax, using simple
        ...
        would be very tedious and not too enlightening.
\end{itemize}
...
applying a signature to a model. $\ms{BootSig}_j$ can be viewed as a precursor to a
...
We define $\ToSSig : \ms{BootSig}$ as the signature for the theory of signatures. We
...
generic way. In particular, almost all type theories in the wild are closed QII
...
\todo{impredicative models are just $\bM_{0, j}$ models!}
\begin{mydefinition}
We define the \textbf{impredicative $\Set_0$ models} of ToS in the following.
These are the same as the standard models which can be used to compute
$\blank^A$, i.e.\ the notion of algebras for each signature. However, the
inductive sorts are fixed to be in $\Set_0$. For some $j$ level, we define an
element of $\ToS_{j+1,j}$, where we have the following:
\begin{alignat*}{3}
  &\Con &&\defn \Set_j \\
  &\Sub\,\Gamma\,\Delta &&\defn \Gamma \to \Delta \\
  &\Ty\,\Gamma &&\defn \Gamma \to \Set_j \\
  ...
  &...&&
\end{alignat*}
Note that in the definition of $\El$ we implicitly lift $a\,\gamma : \Set_0$ to
$\Set_j$ by cumulativity.
