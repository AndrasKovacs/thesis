...
functor. We have the following setup and assumptions.
\begin{itemize}
  \item
  We switch back to the 2LTT-based setup: we have ToS in the outer theory, and
  underlying sorts in algebras are in $\Ty_0$.
  \item
  We write $\llb\blank\rrb$ for the interpretation into the flcwf model $\bM$.
  \item
  We assume that an initial algebra exists for each signature.
  \item
  We extend types in ToS with $\top$ and $\Sigma$, that is, we have $\top :
  \Ty\,\Gamma$ and $\Sigma : (A : \Ty\,\Gamma) \to \Ty\,(\Gamma \ext A) \to
  \Ty\,\Gamma$. It is straightforward to extend the flcwf semantics with these
  type formers; in particular, since ToS is an infinitary QII theory, $\top$ and
  $\Sigma$ follow from the semantics given in Chapter \ref{chap:iqiit}. $\top$
  and $\Sigma$ allows us to use ToS types as signatures (instead of contexts), which
...
\subsubsection{Representing signature}
...
%% \begin{mydefinition} For each $\nu : \Sub\,\Omega\,\Delta$ and $\delta : \Delta^A$, the
%% comma category $\delta/\sem{\nu}$ is represented with $
