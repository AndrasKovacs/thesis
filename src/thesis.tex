...
arguments and implicit quantification. We use $(x : a) \to B$ for inductive
functions, $(x : A) \toe B$ for external functions, but we still write $\Id$ for
...
simple signatures, there is no way to abstract over inductive functions, since
...
algebras yields a lot less encoding overhead than strictifying pullbacks.
...
quotienting with relations. See also \cite[Section~2.2]{ttintt}, where it is
argued that infinitary quotient inductive types extend the base type theory with
some form of constructive choice principles.

Recently, Fiore, Pitts and Steenkamp showed that a class of infinitary quotient
inductive types, called QWI-types, can be reduced to inductive types, quotients
the axiom of weakly initial sets of covers (WISC)
...
universe of propositions. The infinitary ToS syntax is not a QWI-type, since it
is inductive-inductive.  Nevertheless, it is a reasonable conjecture that
infinitary QIITs are also constructible from the WISC principle. We leave this
to future work.

In this section we show constructions of certain fragments of the full ToS
syntax. In the following, we first give a general description of QIIT
constructions, then describe two specific constructions, for a) finitary
inductive-inductive signatures b) closed QII signatures.
...
well-typed. We refer the reader to the Agda formalization \cite{TODO} for these
definitions. In the $\blank^A$ and $\blank^D$ cases, the definitions are simple
enough:
\begin{alignat*}{3}
  &(\J\,P\,\mi{pr}\,p)^A\,\gamma   &&\defn \J\,(\mi{pr}^A\,\gamma)\,(p^A\,\gamma)\\
  &(\J\,P\,\mi{pr}\,p)^D\,\gamma^D &&\defn \J\,(\J\,(\mi{pr}^D\,\gamma^D)\,(p^A\,\gamma))\,(p^D\,\gamma^D)
...
szignatúrák szemantikáját internálisan magában a szignatúrák elméletének a
