...
$\Id$ and $\K$ columns can be unfolded. We get the whole model by filling in
every cell of the unfolded table with a definition. Of course, many of these
cells are equations between equations, hence trivial by UIP.

This setup is very regular and convenient, because we can extract a displayed
ToS model from any column, which may depend on displayed model to the left. The
...
\item By separately taking the eliminators of each column, and referring to previous
      eliminators in each displayed model; e.g.\ referring to the eliminator maps $\blank^A$ in
...
\Tm\,\Gamma\,\U$ in ToS induces an external set of terms as
$\Tm\,\Gamma\,(\El\,a)$.
...
definition this way. Slices models would require the specification of
...
%% In the \emph{slice model} $\ToS/\Omega$, contexts are extensions of
%% $\Omega$ with zero or more entries, and the empty context is simply $\Omega$. Thus,
%% in $\ToS/\Omega$, an internal $\Gamma$-algebra is an element of $\Sub\,\Omega\,\Gamma$.

%% inside the standard $\Set$ model (which yields $\blank^A$), a $\Sub\,\emptycon\,\Gamma$ is
%% actually isomorphic to $\Gamma^A$.
