...
For term algebras, we have to assume that the inner theory supports the theory
of signatures. We are going to have a bit of a name clash here, so we use
$\SigTy_0$ to refer to signature types, and $\SigTm_0$ for terms. That is, we have
\begin{alignat*}{3}
  & \SigTy_0  &&: \Ty_0\\
  & \Con_0   &&: \Ty_0\\
...
\end{alignat*}
together with all constructors and induction principles.

\emph{Remark.} Strictly speaking, if we only want to construct term algebras, we
don't need to assume induction principles. Also, we will see in Section
\ref{TODO} that inner ToS induction is not necessary to show weak initiality of
term algebras; weaker assumptions suffice. In this section, we omit these
generalizations, as the goal is to redo the constructions of Chapter
\ref{chap:simple-inductive-signatures} without making essential changes.

At the same time, we still have ToS in the outer layer. To make the naming
...
by outer induction. Lowering preserves all value constructors.
\begin{alignat*}{3}
