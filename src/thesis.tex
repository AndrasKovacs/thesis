...
\textbf{model constructions}.
\begin{myexample}
For a simple example, we might want to show that constant families are
equivalent to democracy in cwfs. Democracy means

\end{myexample}

%% For example, we can show that constant families
...
$\bs{\top : \Tm\,\Gamma\,\U}$ is the constantly $\top$ morphism, i.e.\ it maps
objects to $\top$ and types to $\lambda\,\_.\,\top$, and maps morphisms and
...
define $\bs{\Sigma\,a\,b : \Tm\,\Gamma\,\U}$ as the evident component-wise
$\Sigma$ of $\ba$ and $\bb$.
\begin{alignat*}{3}
  &(\bs{\Sigma\,a\,b})\,\ulGamma &&\defn (\alpha : \ba\,\ulGamma) \times \bb\,(\ulGamma,\,\alpha)\\
  &(\bs{\Sigma\,a\,b})\,\ulsigma\,(\alpha,\,\beta) &&\defn (\ba\,\ulsigma\,\alpha,\,\bb\,(\ulsigma,\,\refl)\,\beta)\\
  &(\bs{\Sigma\,a\,b})\,\ulA\,\gamma &&\defn (\alpha : \ba\,\ulA\,\gamma) \times \bb\,(\ulA,\,\alpha)\,\gamma
\end{alignat*}
\todo{TODO}
...
as they are straightforward. Equality reflection and $\bs{\refl :}
\bId\,\bt\,\bt$ are also evident.

\subsubsection{Infinitary Function Type}

For $\mi{Ix} : \Set_j$ and $\bb : \mi{Ix} \ra \bTm\,\bGamma\,\bU$, we aim to define
...
\end{alignat*}
The preservation isomorphisms are pointwise inherited from $\bb$. For example,
one direction is defined as follows. Recall that $\emptycon_{\bU} \equiv \top$
and $\ext_{\bU}$ is $\Sigma$.
\begin{alignat*}{3}
