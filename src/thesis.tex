...
\todo{formalize Rec and Ind with spines in Agda. Revise paragraph below}
...
is, the base category together with the terminal object, the $(\Ty_1,\,\Tm_1)$
family and some type formers. This presheaf semantics is well-known in the
literature \cite{TODO}. We give a specification which follows \cite{TODO} most
closely.
...
defined. We denote components of $\hmbbC$ by simply using the component names
from Section \ref{sec:models-of-tts}.
...
\begin{mydefinition}
$A : \Ty_1\,\Gamma$ is a displayed presheaf over $\Gamma$. The
...
\begin{mydefinition}
$t : \Tm_1\,\Gamma\,A$ is a section of the displayed presheaf $A$. This is
...
\begin{mydefinition}
$A[\sigma] : \Ty_{1}\,\Gamma$ is defined as follows, assuming
$A : \Ty_{1}\,\Delta$ and $\sigma : \Sub\,\Gamma\,\Delta$.
\begin{alignat*}{3}
...
\Sub\,(\yon\,i)\,\Gamma $ to denote transporting right-to-left along
the Yoneda lemma. In the other direction, we don't need a notation as it is,
since from $\sigma : \Sub\,(\yon\,i)\,\Gamma$ we get $\sigma\,\id :
|\Gamma|\,i$.
\end{notation}
...
\begin{alignat*}{3}
  & \K &&: \Con \to \{\Gamma : \Con \} \to \Ty_1\,\Gamma\\
  & |\K\,\Delta|\,\{i\}\,\gamma\,&&\defn |\Delta|\,i \\
  & \delta\lab f \rab &&\defn \delta \lab f \rab
\end{alignat*}
With this definition, we actually have $\Tm_1\,\Gamma\,(\K\,\Delta) =
...
\begin{mydefinition}
From any $A : \Set$, we get $\K\,A : \Ty_1\,\Gamma$. This can be used to
...
non-dependent type, it is helpful to define it as $\U : \Con$ such that
$\Sub\,\Gamma\,\U \simeq \Ty_1\,\Gamma$.  The usual universe can be derived from
...
this should be isomorphic to $\Ty_1\,(\yon\,i)$, so we take this as definition:
\begin{alignat*}{3}
  & \U &&: \Con\\
  &|\U|\,i &&\defn \Ty_1\,(\yon\,i)\\
...
\end{alignat*}
\end{mydefinition}
In the $A \lab f \rab$ definition, we substitute $A : \Ty_1\,(\yon\,i)$ with
$\yon f : \Sub\,(\yon j)\,(\yon i)$ to get an element of $\Ty_1\,(\yon j)$.
The required $\Sub\,\Gamma\,\U \simeq \Ty_1\,\Gamma$ is straightforward, so we
omit the definition.
...
\begin{alignat*}{3}
  & \Sigma  &&: (A : \Ty_1\,\Gamma) \to \Ty_1\,(\Gamma\ext A) \to \Ty_1\,\Gamma\\    & |\Sigma\,A\,B|\,\gamma && \defn (\alpha : |A|\,\gamma) \times |B|\,(\gamma,\,\alpha)\\
...
Restriction is functorial since it is defined as $\Tm_1$ substitution. The definitions
...
$\Con$ and $\Tm_{\mbbC}$ is an element of $\Ty_1\,\Ty_{\mbbC}$. With this is mind,
we can give the following definitions:
\begin{alignat*}{3}
  & \Ty_{0}\,\Gamma &&\defn \Sub\,\Gamma\,\Ty_{\mbbC}\\
  & \Tm_{0}\,\Gamma\,A &&\defn \Tm_1\,\Gamma\,(\Tm_\mbbC[A])
\end{alignat*}
...
\begin{alignat*}{3}
  & \Lift\,\,: \Ty_{0}\,\Gamma \to \Ty_{1}\,\Gamma \\
  & \Lift\!A \defn \Tm_{\mbbC}[A]
\end{alignat*}
This yields $\Tm_{0}\,\Gamma\,A \equiv
\Tm_{1}\,\Gamma\,(\Lift\!A)$, which is the second condition from
...
$(\Ty_0,\,\Tm_0)$? This depends on what we mean by ``type former''. Capriotti
examined this question in some generality \cite{TODO}, although he did not make
the notion of type formers fully precise. Some of the inductive signatures in
this thesis or in the more recent HOAS literature \cite{TODO} could be possibly
used to revisit this question. We don't do this in this thesis. We can give
though a quick illustration and explanation.

Generally, a type former in $\mbbC$ transfers to $\hmbbC$ if the specification
of the type former can be already given in the internal language of $\hmbbC$; if
the type former ``always has been'' in $\hmbbC$ to begin with. To be describable
in $\hmbbC$, a type former needs to be natural with respect to
substitution. This is also a core idea of HOAS: when working in $\hmbbC$,
everything is natural, and we can omit boilerplate related to contexts and
substitutions.

For example, if we switch to a more convenient Agda-like surface syntax for
2LTT, we may have $\Ty_{\mbbC} : \U$ and $\Tm_{\mbbC} : \Ty_{\mbbC} \to \U$,
...
\begin{alignat*}{3}
  &\Pi_0           : (A : \Ty_{\mbbC}) \to (\Tm_{\mbbC}\,A \to \Ty_{\mbbC}) \to \Ty_{\mbbC}\\
  &(\app_0,\,\lam_0) : \Pi_0\,A\,B \simeq ((x : \Tm_{\mbbC}\,A) \to \Tm_{\mbbC}(B\,x))
\end{alignat*}
...
There is a useful simplification in the standard presheaf model, in cases where
we have functions of the form $\Pi\,(\Lift A)\,B$. First we look at the
simply-typed case with presheaf exponentials. We need some observations about
