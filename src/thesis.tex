...
We cover almost every higher inductive definition in the literature in a direct
manner, without relying on circuitous encodings.
...
types. If our goal is to add computationally adequate HIITs to a theory (or
implementation), the current chapter is not immediately applicable. In a cubical
setting we would need to reformulate both signatures and semantics. However,
the current work should be still helpful as a guideline, and a point of
...
chapter, we extend and refine these source in the following ways.
\begin{itemize}
\item
