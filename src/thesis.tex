...
Only strictly positive signatures are expressible.
...
the signatures in this thesis. The simplest example is rose trees, assuming
...
\cite[Section~1.12.1]{hottbook}:
\begin{alignat*}{3}
  & \ms{J} :\,&&\{A : \Ty_0\}\{x : A\}(B : (y : A) \to x = y \to \Ty_0) \to
  B\,x\,\refl\\ & &&\to (y : A)(p : x = y) \to B\,y\,p\\ & \rlap{$\ms{J\beta} :
    \ms{J}\,B\,\mi{br}\,x\,\refl \equiv \mi{br}$}
\end{alignat*}
The following operations are defined in the standard way \cite[Section~2]{hottbook}.
\begin{itemize}
  \item Path inversion $\blank^{-1} : x = y \to y = x$.
  \item Path composition $\blank\!\sqcdot\!\blank : x = y \to y = z \to x = z$.
  \item Transport $\tr : (P : A \to \Ty_0) \to (p : x = y) \to P\,x \to P\,y$.
  \item Path lifting $\ap : (f : A \to B) \to x = y \to f\,x = f\,y$.
  \item Dependent path lifting $\apd : (f : (x : A) \to B\,x) \to (p : x = y) \to \tr\,B\,p\,(f\,x) = f\,y$.
\end{itemize}

\section{Strict Signatures}

%% We also assume \emph{strong function extensionality} for $\blank\!=\!\blank$
%% \cite[Section~2.8]{hottbook}. This means that the function $\ms{happly} : (f =
%% g) \to ((x : A) \to f\,x = g\,x)$ is an equivalence, and the inverse of
%% $\ms{happly}$ is taken to be the extensionality principle $\ms{funext}$.
...
%% $\Id$ is intensional, with the following rules:
%% \begin{alignat*}{3}
%%   & \J &&: \{t : \Tm\,\Gamma\,(\El\,a)\}
%%          (b : \Tm\,(\Gamma\ext(u : \El\,a)\ext(p : \El\,(\Id\,t\,u)))\,\U)\\
%%   & &&\to \Tm\,\Gamma\,(\El\,(b[u \mapsto t,\,p \mapsto \refl]))\\
%%   & &&\to \{u : \Tm\,\gamma\,(\El\,a)\}
%%        (p : \Tm\,\Gamma\,(\El\,(\Id\,t\,u)))
%%   \to \Tm\,\Gamma\,(\El\,(b[u \mapsto u,\,p \mapsto p]))\\
%%   & \refl &&: \Tm\,\Gamma\,(\El\,(\Id\,t\,t))\\
%%   & \J\beta &&: \J\,b\,\mi{pr}\,\refl \equiv \mi{pr}
%% \end{alignat*}
%% Note that we can only eliminate from $\Id$ to types in $\U$. Eliminating to
%% arbitrary types won't be supported in the semantics. The reason is that strict
%% preservation equations in the semantics force types to return in $\Set$.
...
also not discuss fine-grained sizing or cumulativity of algebras.
  ...
  %% \item
  %%   We additionally have an intensional large identity type $\ID : \Tm\,\Gamma\,A \to
  %%   \Tm\,\Gamma\,A \to \Ty\,\Gamma$, with the following rules:
  %%   \begin{alignat*}{3}
  %%     & \J &&: \{t : \Tm\,\Gamma\,A\}
  ...
  %%     & \J\beta &&: \J\,P\,\mi{pr}\,\refl \equiv \mi{pr}
  %%   \end{alignat*}
...
\item
  ...
  be possible. For HIITs, we can't do that, since the inner theory can't
  reasonably internalize the ToS.
\end{itemize}

%% \begin{myexample}
%% The torus signature \cite[Section~6.6]{hottbook} is a relatively simple example which involves
%% $\J$.  For $p_1 : \Tm\,\Gamma\,(\El\,(\Id\,t\,u))$ and $p_2 :
%% \Tm\,\Gamma\,(\El\,(\Id\,u\,v))$, we abbreviate their composition as $p_1 \sqcdot p_2 :
%% \Tm\,\Gamma\,(\El\,(\Id\,t\,v))$, defined using $\J$ in the evident way.
%% \begin{alignat*}{3}
%%   &\ms{T}^2  &&: \U\\
%%   &\ms{b}    &&: \El\,\ms{T}^2\\
...
%% \begin{myexample}
%% The large $\ID$ type lets us express ``sort equivalences''. For example, a signature for
%% integers can be compactly written as follows \cite{hit-integers}:
%% \begin{alignat*}{3}
%%   &\ms{Int}  &&: \U\\
%%   &\zero     &&: \El\,\ms{Int}\\
%%   &\p        &&: \ID\,\ms{Int}\,\ms{Int}
...
%% \subsubsection{Limitation: elimination from external parameters to signature terms}

%% Our signatures support every higher inductive type from the homotopy type theory
%% book, with the exception of the ``hubs-and-spokes'' HITs
%% \cite[Section~6.7]{hottbook}.  This involves abstracting over some external $x :
%% \ms{S}^1$ (a point of the circle), then referring to a ToS term which is
%% computed by elimination on $x$. This is also not permitted in our setup, because
%% signature terms live in the outer theory of 2LTT, and external parameters are in
%% $\Ty_0$.
...
In the following, we present the AMDS interpretations in order, in a compact and
tabular manner. First we list the components of the ToS, as a reference for the
interpretations. We discuss the semantics afterwards.
...
\begin{alignat*}{3}
  ...
  &(\Id\,t\,u)^D\,\gamma^D &&\defn \lambda\,(p : t^A\,\gamma = u^A\,\gamma).\,\tr\,(a^D\,\gamma^D)\,p\,(t^D\,\gamma^D) = u^D\,\gamma^D\\
...
In \textbf{algebras} and \textbf{displayed algebras} there isn't any
...
\begin{alignat*}{3}
  ...
  & \times\,&&(\mi{loop^D} &&: \tr\,S^{1D}\,\mi{loop}\,\mi{base^D} = \mi{base^D})\\
  & &&\\
...
semantics. We don't explore this in the current chapter.
...
and $\Tm$ returning in $\Set$) together with the following type formers.
\begin{itemize}
\item An universe $\U$ with decoding $\El$.
\item $\U$ is closed under $\Id$, with elimination principle $\J$ targeting
  $\U$. The $\beta$-rule is given up to $\Id$:
  \begin{alignat*}{3}
  & \J &&: \{t : \Tm\,\Gamma\,(\El\,a)\}
         (b : \Tm\,(\Gamma\ext(u : \El\,a)\ext(p : \El\,(\Id\,t\,u)))\,\U)\\
  & &&\to \Tm\,\Gamma\,(\El\,(b[u \mapsto t,\,p \mapsto \refl]))\\
  ...
  \to \Tm\,\Gamma\,(\El\,(b[u \mapsto u,\,p \mapsto p]))\\
  & \refl &&: \Tm\,\Gamma\,(\El\,(\Id\,t\,t))\\
  &\J\beta &&: \Tm\,\Gamma\,(\El\,(\Id\,(\J\,b\,\mi{pr}\,\refl))\,(\El\,\mi{pr}))
  \end{alignat*}
\item
  $\U$ is also closed under $\top$, $\Sigma$, and $\Piinf$. Of these, $\top$ is
  specified with a strict isomorphism $\Tm\,\Gamma\,(\El\,\top) \simeq \top$,
  ...
  several equivalent formulations of equivalence; we pick bi-invertible
  equivalences here for presentation.

  \begin{mydefinition}
    An \textbf{equivalence} of $a,\,b : \Tm\,\Gamma\,\U$ consists of the
    following
