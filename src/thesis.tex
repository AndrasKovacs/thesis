...
signatures from old ones. We describe two specific applications in the
following.
...
\end{alignat*}
Above, we use $\id_{\ms{tos}} : \Tm\,(\emptycon\ext(\gamma : \Gamma^A_{\ms{tos}}))\,(\Gamma^M_{\ms{tos}}\,[\gamma_0 \mapsto \gamma,\,\gamma_1 \mapsto \gamma])$, which also comes from the signature-based
semantics.

\subsubsection{Unfolded signatures for internal algebras}

Consider algebras which are internal to some category of algebras. For example,
double categories are categories internally to the category of categories. We can
always
