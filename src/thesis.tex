...
As in the term algebra construction, we pick $\Omega : \Sig_j$, use $\blank^A$
to refer to $\blank^A_{j+1}$, and also use $\blank^M$ for $\blank^M_{j+1}$.  We
assume $\omega : \Omega^A$. The goal is to exhibit a morphism from the term
algebra $\Omega^T\,\id$ to $\omega$. We define $\blank^R$ by induction on
$\syn_j$.
\begin{alignat*}{3}
...
\item
  ...
  \[ A^M : A^A\,\gamma_0 \to A^A\,\gamma_1 \to \Gamma^M\,\gamma_0\,\gamma_1 \to \Set_{j+1} \]
...
\textbf{contexts}, we again simply recurse:o
\begin{alignat*}{3}
  &\emptycon^R\,\nu           &&\defn \tt\\
  &(\Gamma \ext A)^R(\nu,\,t) &&\defn (\Gamma^R\,\nu,\,A^R\,\nu\,t)
\end{alignat*}
...
follows by $a^T\,\id : t^A\,(\Omega^T\,\id) \equiv t$.

For \textbf{the identity type}, we need to show:
\[
...
%% (Id t u)ᴿ : (ν : Sub Ω Γ)(e : Tm Γ (Id t[ν] u[ν])) → (Id t u)ᴹ (tᴬ (Ωᵀ id)) (tᴬ ω) (Γᴿ ν)
%%           : (ν : Sub Ω Γ)(e : Tm Γ (Id t[ν] u[ν])) → tᴹ (Γᴿ ν) ≡ uᴹ (Γᴿ ν)
