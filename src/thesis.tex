...
\section{Recovering AMDS Interpretations}
...
ToS model from any column, which may depend on columns to the left. The
whole model is the total model of all columns. For example, the $\Con$ column
doesn't depend on anything, so it's a plain model. The $\Ty$ column is
displayed over $\Con$. The $\Tm$ column is displayed over the total model of
$\Con$ and $\Ty$, but it does not actually depend on $\Sub$.
...
signatures, and their recursors and eliminators. We make some generalizations,
in comparison to what we did with simple signatures.

\begin{itemize}
  \item  We assume that $\Ty_0$ supports a model of ToS. As before,
...
We fix a $\Omega : \Con_1$. We define $\blank^T$ by induction on the outer
...
equations are given by UIP. We omit most cases for substitutions and terms.
