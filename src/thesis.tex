...
$u^M\,\gamma^M : X^M\,(u^A\,\gamma_0) = u^A\,\gamma_1$. We again pack up
$\Gamma^M$ with the interpretation of $\iota$, to get notions of algebra
morphisms:
\begin{alignat*}{3}
...
\begin{alignat*}{3}
  & \Mor\,\NatSig\,(X_0,\,\mi{zero_0},\,\mi{suc_0})\,(X_0,\,\mi{zero_1},\,\mi{suc_1}) \equiv \\
  & \hspace{2em}
    (X^M : X_0 \to X_1) \times (X^M\,\mi{zero_0} = \mi{zero_1}) \times ((x : X_0) \to X^M\,(\mi{suc_0}\,x) = \mi{suc_1}\,(X^M\,x))
\end{alignat*}
\end{myexample}

\paragraph{Morphisms vs.\ logical relations.}
The above $\blank^M$ interpretation can be viewed as a special case of logical
...
to be a function. Let us consider now the more general relational interpretation
for types:
\begin{alignat*}{3}
...
an arbitrary $A$ type, we would only have a black-box $A^M\,X^M : A^A\,X_0 \to
A^A\,X_1 \to \Set$ relation, and we would not be able to singleton contract.

In Chapter \ref{chap:fqiit} we will expand on this. As a preliminary summary:
