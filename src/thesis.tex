...
\chapter{Finitary Quotient Inductive-Inductive Signatures}
\label{chap:fqiit}
...
\label{sec:fqiit-levels}

\todo{Mention this earlier in the chapter?}
...
\end{alignat*}
Similarly as before, we make use of $\nu^T\,\id$, $u^T\,\id$, $a^E\,\nu$ and $a^T\,\nu$ to type-check the definition.
...
\chapter{Infinitary Quotient Inductive-Inductive Signatures}
\label{chap:iqiit}
...
example for this would be set truncation for some $A : \Ty_0$:
\begin{alignat*}{3}
  &|A|_0      &&: \U\\
  &\mi{embed} &&: A \toe \El\,|A|_0 \\
  &\mi{trunc} &&: (x\,y : |A|_0)(p\,q : \Id\,x\,y) \to \El\,(\Id\,p\,q)
\end{alignat*}
However, this will end up being redundant in our semantics, since we assume UIP,
...
\todo{Section on model constructions using signature morphisms}
...
the strict semantics. What about working in homotopy type theory though: instead
of working with isofibrations in an extensional setting, could we work with
univalent structures in HoTT? In other words, work with univalent categories of
algebras, and univalent displayed categories over them \cite{displayedcats}. A
major benefit of the univalent setting is that we would get a \emph{structure
identity principle} \cite{TODO} out of the semantics, which says that for algebras,
...
\section{Model of the Theory of Signatures}
...
Contexts are the same as before, namely flcwfs, so we start the exposition with
substitutions.
...
We aim to define $\bs{\blank[\blank] : \Ty\,\Delta \ra \Sub\,\Gamma\,\Delta \ra
  \Ty\,\Gamma}$, such that $\bs{A[\id]} = \bA$ and $\bs{A[\sigma\circ\delta]} =
\bs{A[\sigma][\delta]}$. As before, the underlying sets are given by simple
composition:
\begin{alignat*}{3}
  & \Con_{\bs{A[\sigma]}}\,\ulGamma && := \Con_{\bA}\,(\bsigma\,\ulGamma)\\
  & \Sub_{\bs{A[\sigma]}}\,\Gamma\,\Delta\,\ulsigma && :=
    \Sub_{\bA}\,\Gamma\,\Delta\,(\bsigma\,\ulsigma)\\
  & \Ty_{\bs{A[\sigma]}}\,\Gamma\,\ulA && :=
      \Ty_{\bA}\,\Gamma\,(\bsigma\,\ulA)\\
  & \Tm_{\bs{A[\sigma]}}\,\Gamma\,A\,\ult && :=
      \Tm_{\bA}\,\Gamma\,A\,(\bsigma\,\ult)
\end{alignat*}
...
essentially the same reasoning as before, as all the previous transports still
work. Context and type formers are given by coercing $\bA$ structures along
$\bsigma$ preservation isomorphisms. For example:
\begin{alignat*}{3}
  &\emptycon_{\bs{A[\sigma]}} && :=
    \coe\,\bsigma_{\emptycon}^{-1}\,\emptycon_{\bA}\\
  &\Gamma\ext_{\bs{A[\sigma]}}A && :=
...
isomorphisms, e.g.\ we have $\coe_{\bs{A[\sigma]}}\,\ulsigma\,\Gamma
:= \coe_{\bA}\,(\bsigma\,\ulsigma)\,\Gamma$.

Functoriality of type substitution, i.e.\ $\bs{A[\id]} = \bA$ and
$\bs{A[\sigma\circ\delta]} = \bs{A[\sigma][\delta]}$, follows
