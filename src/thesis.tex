...
\newcommand{\bK}{\boldsymbol{\mathsf{K}}}
...
$\mi{Hom}$. When we give semantics to this signature in Section \ref{TODO}, we
will get as algebras presheaves from $\mbbC$ to the category of inner
...
\subsection{Discussion of Semantics}

\subsubsection{Flcwfs For Free}
...
In other cases, the flcwf ``for free'' has to be extended with extra
conditions. This often happens if the objects in the model have an internal
notion of ``equivalence'' which has to be respected by types.
\begin{itemize}
...
In all these cases, the semantic objects have FQII signatures. We can take their
flwcfs internally to $\bs{\Set}$ and add fibrancy conditions. The cubical set
model is presented exactly in this way in \cite{cubical}, using displayed
...
\subsubsection{Variations of the Semantics}
...
\\\\
\indent One use case of $\Sub$ is to specify \textbf{ornaments} \cite{TODO}, i.e.\ ways
...
\\\\
\indent In a broader context, ToS provides a synthetic language for specifying
\textbf{model constructions}. For example, we can show that constant families
and democracy are equivalent, by defining an invertible $\Sub$ between
signatures of $\ms{cwf}+\K$ and $\ms{cwf^{dem}}$, and getting an isomorphism of
categories of models.

In many cases, this does not work, since ToS is rather restricted as a
language. In Chapter \ref{chap:iqiit}, we present a more expressive ToS which
is sufficient to rephrase many syntactic models \cite{next700} as signature
morphisms, thus morphisms of categories of models.

\subsubsection{Recovering AMDS Interpretations}
...
%% \begin{mydefinition} From now on, we use $\blank^A$, $\blank^M$, $\blank^D$ and $\blank^S$ to refer
%% to the families of functions that we described above.
%% \end{mydefinition}
...
\section{Moving to the Inner Theory \& Refining Universe Levels}

In the rest of the chapter, we discuss term algebras and their recursors and
eliminators. Before we do that, we make two significant modifications to the
setup.
\\\\
...
self-describing signatures in Chapter \ref{TODO}. In the following, we describe
the new universe setup, and adapt the previously described signatures and
semantics to it.
...
\cite{sterling2019algebraic,kovacs2021generalized}, which requires a rather
large amount of explicit lifting annotation.
...
Note that we have an \emph{invariant} rule for function domain types. This is in
order to match Coq and \cite{timany18cumulative}, and also because we will not
need a contravariant rule in any case.
...
in $\Set_i$, and $\Pie$ abstracts over $\Set_j$. \emph{Remark:} by the
cumulativity rules, we have that $\ToS_{i,j} \leq \ToS_{i+1,j}$.
\end{mydefinition}
...
%% signatures, together with their recursors and eliminators.  First, we review our
%% assumptions in the inner theory.

%% \subsection{Inner Assumptions}

%% \begin{notation} As before, we use $\SigTy_i$ and $\SigTm_i$ for disambiguation, and
%% now we additionally use $\SigU_i : \SigTy_i\,\Gamma$ for the universe in
%% ToS$_i$, to disambiguate it from the additional universes that we shall
%% introduce shortly.
%% \end{notation}

%% So far, we have been consistently ignoring universe levels, but now we shall
%% make them more explicit. Perhaps we could still gloss over these details, but
%% there are some relevant details which were far from obvious to the author.
%% Consider external function types in $\SigTy_0$:
%% \begin{alignat*}{3}
%%   &\SigTy_0 &&: \Con_0 \to \Ty_0\\
%%   &\Pie &&: (A : \Ty_0) \to (A \to \SigTy_0\,\Gamma) \to \SigTy_0\,\Gamma
%% \end{alignat*}
%% First, this type makes little sense as part of an inner inductive
%% definition, because it entails ``storing'' an proper inner type (not in any
%% universe) in an inner inductive constructor. Traditionally, we cannot abstract
%% over arbitrary types in type theories; we can assume \emph{terms} in contexts
%% with certain types, but cannot assume \emph{types}. Hence, we would not be able
%% to write down the induction method for $\Pie$ purely in an inner type theory.

%% The solution is to use an inner universe, say $\U$, instead of $\Ty_0$. But we
%% still have to avoid the problem that $\SigTy_0\,\Gamma : \U$, but $\Pie$
%% abstracts over $\U$, i.e.\ ``contains'' an element of $\U$. Together with the
%% induction principle for inner ToS, this would let us recover $\U$-in-$\U$, hence
%% an inconsistency. If we \emph{don't} assume induction, then it might be
%% sensible to consider specific models where levels are not bumped up; but we do
%% need induction for constructing recursors and eliminators.

%% Hence, the abstracted $A$ in $\Pie$ needs to be smaller than $\SigTy_0$.  Also,
%% we need $\SigTy_0$ to return in an actual inner universe instead of $\Ty_0$,
%% because we cannot compute types in $\Ty_0$ by inner elimination.

%% \begin{itemize}
%%   \item We assume $\U_0$, $\U_1$ as sub-universes of $\Ty_0$. These
%%     are assumed to be \emph{cumulative}, with $\U_0 < \U_1 < \Ty_0$. This
%%     means that any type in a smaller universe may be implicitly lifted to a
%%     larger one. Formally, the algebraic specification of cumulativity would
%%     involve a significant amount of detail, e.g.\ lifting would be an explicit
%%     operation on types \cite{kovacs2021generalized}. We instead use a Coq-like
%%     notation, where cumulativity is implicit. We also omit $\Tm_0$ everywhere.
%%   \item
%%     We assume an inner ToS (or: ToS$_0$) where $\Con_0$, $\SigTy_0$, $\SigTm_0$ and $\Sub_0$ all
%%     return in $\U_1$, and we have the following type for $\Pie$:
%%     \[
%%       \Pie : (A : \U_0) \to (A \to \SigTy_0\,\Gamma) \to \SigTy_0\,\Gamma
%%     \]
%%     We assume that ToS$_0$ supports induction, where we may eliminate into $\Ty_0$, hence
%%     also into any sub-universe of $\Ty_0$.

  %%   We assume that $\Ty_0$ supports a model of ToS. As before,
  %%        we use $\SigTy_i$ and $\SigTm_i$ in the following. Thus, we have $\Con_0 :
  %%        \Ty_0$, $\Sub_0 : \Con_0 \to \Con_0 \to \Ty_0$, $\SigTy_0 : \Con_0 \to \Ty_0$
  %%        and $\SigTm_0 : (\Gamma : \Con_0) \to \SigTy_0\,\Gamma \to \Ty_0$. At the same
  %%        time, we still have $\Con_1$, $\Sub_1$ and so on in the outer theory. Note that
  %%        at this point, we \emph{do not assume} any recursion or induction principle for
  %%        the inner ToS model.
  %% \item  We define lowering maps $\down$ similarly as in Definition \ref{def:simple-lowering}.
  %%        Again, these maps preserve all structure.
%% \end{itemize}

In this section we proceed with the construction of term algebras for FQII
signatures, together with their recursors and eliminators.

\begin{mydefinition} [\textbf{Term algebras}]
\label{sec:fqiit-term-algebras}
We fix $\Omega : \Sig_j$ for some $j$ level. We define $\blank^T$ by induction on the outer
ToS. Formally, we need a displayed ToS model, but we instead present the
resulting eliminator, which is perhaps easier to read. The underlying functions
have the following types.
\begin{alignat*}{3}
  &\blank^T &&: (\Gamma : \Con_1)&&(\nu : \Sub_0\,(\down\!\Omega)\,(\down\!\Gamma)) \to \Gamma^A\\
  &\blank^T &&: (\sigma : \Sub_1\,\Gamma\,\Delta)&&(\nu : \Sub_0\,(\down\!\Omega)\,(\down\!\Gamma)) \to \Delta^T\,(\down\!\sigma \circ \nu) \equiv \sigma^A\,(\Gamma^T\,\nu)\\
  &\blank^T &&: (A : \SigTy_1\,\Gamma)&&(\nu : \Sub_0\,(\down\!\Omega)\,(\down\!\Gamma)) \to \SigTm_0\,(\down\!\Omega)\,((\down\!A)[\nu])
  \to A^A\,(\Gamma^T\,\nu)\\
  &\blank^T &&: (t : \SigTm_1\,\Gamma\,A)&&(\nu : \Sub_0\,(\down\!\Omega)\,(\down\!\Gamma)) \to A^T\,\nu\,((\down\!t)[\nu]) \equiv t^A\,(\Gamma^T\,\nu)
\end{alignat*}
...
\SigTm_0\,\Gamma\,\U$ in ToS induces an external type of terms as
$\SigTm_0\,\Gamma\,(\El\,a)$.

We can view the generalization from $\emptycon$ to arbitrary $\Omega$ as
switching from working in a given model $\ms{M}$ of ToS, to working in the
\emph{slice model} $\ms{M}/\Omega$, where contexts are $\Omega$ extended with
zero or more entries. And in $\ms{M}/\Omega$, we have an $\Omega$-algebra quite
trivially, by taking the identity morphism $\id :
\Sub\,\Omega\,\Omega$\footnote{$\Sub_{\ms{M}/\Omega}\,\emptycon\,\Omega$ is
isomorphic to, but not strictly the same as
$\Sub_{\ms{M}}\,\Omega\,\Omega$.}. Hence, term algebras arise by first taking
the trivial internal algebra $\id$ in $\ms{M}/\Omega$, then converting it to an
external algebra as $\Omega^T\,\id : \Omega^A$.
...
$\Sub_0\,(\down\!\Omega)\,(\down\!\Gamma\,\,\ext\down\!A)$, since any $\nu$ with
...
\begin{alignat*}{3}
  ...
  &(\Gamma \ext A)^T(\nu,\,t) &&\defn (\Gamma^T\,\nu,\,A^T\,\nu\,t)
\end{alignat*}
\textbf{Type substitution} is as follows. This is well-typed by
$\sigma^T\,\nu : \Delta^T\,(\down\!\sigma \circ \nu) \equiv \sigma^A\,(\Gamma^T\,\nu)$.
\[ (A[\sigma : \Sub_1\,\Gamma\,\Delta])^T\,\nu\,t \defn A^T\,(\down\!\sigma\circ\nu)\,t \]
For the \textbf{universe}, note that $\U^A\,\gamma \equiv \Ty_0$.  As we
mentioned before, this is the key part when we map from internal sorts to
$\Ty_0$.
\begin{alignat*}{3}
  &\U^T : (\nu : \Sub_0\,(\down\!\Omega)\,(\down\!\Gamma)) \to \SigTm_0\,(\down\!\Omega)\,\U \to
          \Ty_0\\
  &\U^T\,\nu\,a \defn \SigTm_0\,\Omega\,(\El\,a)
\end{alignat*}
For $\El$, we have to define
\[
  (\El\,a)^T : (\nu : \Sub_0\,(\down\!\Omega)\,(\down\!\Gamma))
          \to \SigTm_0\,(\down\!\Omega)\,(\El\,((\down\!a)[\nu]))
          \to a^A\,(\Gamma^T\,\nu)
\]
but since $a^T\,\nu : \SigTm_0\,(\down\!\Omega)\,(\El\,((\down\!a))[\nu])
      \equiv a^A\,(\Gamma^T\,\nu)$, we have
\begin{alignat*}{3}
  &(\El\,a)^T : (\nu : \Sub_0\,(\down\!\Omega)\,(\down\!\Gamma))
          &&\to \SigTm_0\,(\down\!\Omega)\,(\El\,((\down\!a)[\nu]))\\
...
equation is $a^T\,\id : \SigTm_0\,(\down\!\Omega)\,(\El\,(\down\!\,a)) \equiv
...
\begin{alignat*}{3}
  &(\Id\,t\,u)^T : (\nu : \Sub_0\,(\down\!\Omega)\,(\down\!\Gamma))
    &&\to \SigTm_0\,(\down\!\Omega)\,(\Id\,((\down\!t)[\nu])\,((\down\!u)[\nu]))\\
  & &&\to t^A\,(\Gamma^T\,\nu) \equiv u^A\,(\Gamma^T\,\nu)
\end{alignat*}
We know by equality reflection that $(\down\!t)[\nu] \equiv (\down\!u)[\nu]$, and
we also get
\begin{alignat*}{3}
  &t^T\,\nu &&: A^T\,\nu\,((\down\!t)[\nu]) &&\equiv t^A\,(\Gamma^T\,\nu)\\
  &u^T\,\nu &&: A^T\,\nu\,((\down\!u)[\nu]) &&\equiv u^A\,(\Gamma^T\,\nu)
...
\begin{alignat*}{3}
  &(\Pi\,a\,B)^T : (\nu : \Sub_0\,(\down\!\Omega)\,(\down\!\Gamma))
                 &&\to \SigTm_0\,(\down\!\Omega)\,(\Pi\,((\down\!a)[\nu])\,((\down\!B)[\nu\circ\p,\,\q]))\\
  &              &&\to (\alpha : a^A\,(\Gamma^T\,\nu)) \to B^A\,(\Gamma^T\,\nu,\,\alpha)\\
  &\rlap{$(\Pi\,a\,B)^T\,\nu\,t \defn \lambda\,\alpha.\,B^T\,(\nu,\,\alpha)\,(t\,\alpha)$}
\end{alignat*}
This is well-typed by $a^T\,\nu :
\SigTm_0\,(\down\!\Omega)\,(\El\,((\down\!a))[\nu]) \equiv
...
\begin{alignat*}{3}
  &(\Pie\,A\,B)^T : (\nu : \Sub_0\,(\down\!\Omega)\,(\down\!\Gamma))
                 &&\to \SigTm_0\,(\down\!\Omega)\,(\Pie\,A\,(\lambda\,\alpha.\,\down\!(B\,\alpha)))\\
...
\begin{mydefinition} For $\Omega : \Con_1$ signature, the corresponding \textbf{term algebra}
is given as $\Omega^T\,\id : \Omega^A$.
\end{mydefinition}

\subsection{Weak Initiality}

In the following we investigate sufficient conditions for having term algebras
which are weakly initial. For this though, we first have to look universe sizes.
We have done a decent job so far at ignoring universes, but now we really have
to pin down some details.

\subsubsection{Universe Sizes}
First, consider external function types in $\SigTy_0$:
\[
  \Pie : (A : \Ty_0) \to (A \to \SigTy_0\,\Gamma) \to \SigTy_0\,\Gamma
\]
Now, if we view $\Pie$ as part of an inductive signature, we have to problem
that $\SigTy_0\,\Gamma : \Ty_0$, but $\Pie$ abstracts over $\Ty_0$,
i.e.\ ``contains'' an element of $\Ty_0$.

For comparison, an inductive constructor in Agda which abstracts over $(A :
\Set)$ can only return in a sort which is in $\Set_1$. Failure to bump levels,
together with the induction principle for the type in question would let us
recover $\Set : \Set$, hence an inconsistency. If we \emph{don't} assume
induction, then it might be fine and sensible to consider specific models where
levels are not bumped up. But we're aiming for term algebras that are weakly or
strongly initial, so we don't want to run into inconsistent induction
principles.

The simplest way to rectify ToS with $\Pie$ is the following: we assume a
Tarski-style sub-universe of $\Ty_0$, with $\ty_0 : \Ty_0$ and $\tm_0 :
\Tm_0\,\ty_0 \to \Ty_0$, and have $\Pie$ abstract over $a : \ty_0$. We can view
$(\ty_0,\,\tm_0)$ as a parameter to ToS. For instance, we can just define
$\ty_0$ to be $\Bool_0$, and define $\tm_0$ by large elimination. In this case,
we have signatures which can refer to at most two external types.

\subsection{Church Encoding}
\subsection{Awodey-Frey-Speight Encoding}
\section{Left Adjoints of Signature Morphisms}
\label{sec:left-adjoints}
