...
module _ (Ix : Set) where
  ...
  data Tm (Γ : Con) (i : Ix) : Set
  data Sp (Γ : Con) (i : Ix) : Ty → Set

  data Tm Γ i where
    _$_ : ∀ {A} → Var Γ A → Sp Γ i A → Tm Γ i

  data Sp Γ j where
    ε    : Sp Γ j (sort j)
    app  : ∀ {i A} → Tm Γ i → Sp Γ j A → Sp Γ j (ind i A)
    appʰ : ∀ {A i B} → (∀ a → Tm Γ (i a)) → Sp Γ j B → Sp Γ j (hind A i B)
    appᵉ : ∀ {A B} → ∀ a → Sp Γ j (B a) → Sp Γ j (ext A B)

  Motive : Con → Set
  Motive Γ = ∀ i → Tm Γ i → Set
